...
def get_jacobian_from_joint(urdfname,jointq,flag):
    ...
    # 注意这里是因为从base_link算起，base_link到base要绕z轴转180度，实际ur读速度都是按base坐标读的。因此需要绕z轴转180度。如果改成base会报错。0826 yxj
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        J_inv = np.linalg.pinv(J[0:3,:]) # only compute position!!
        N_pos = np.eye(6)-np.dot(J_inv,J[0:3,:])

        J_inv = np.linalg.pinv(J[3:6,:]) # only compute orientation!!
        N_ori = np.eye(6)-np.dot(J_inv,J[3:6,:])
        ...
        if flag_initialized==0:# 先回到初始位置，movej的t参数是不需要的，这里给0
            ...
            t_ready = t
        elif flag_initialized==2  and t-t_ready<10:# 
            d = [-0.1*math.sin(math.pi*(t-t_ready)),-0.1*math.sin(math.pi*(t-t_ready)),-0.1*math.sin(math.pi*(t-t_ready)),\
                -0.1*math.sin(math.pi*(t-t_ready)),-0.1*math.sin(math.pi*(t-t_ready)),-0.1*math.sin(math.pi*(t-t_ready))]# 对6个关节都有disturbance
            # v = -np.dot( J_inv , drdot-Kp*(r-dr) ) + np.dot( N_ori , np.dot(np.linalg.inv(Cd), np.reshape(d,[6,1])) )
            v = np.dot( N_ori , np.dot(np.linalg.inv(Cd), np.reshape(d,[6,1])) )
            # print('r-dr',r-dr)
            # print('drdot-(r-dr)',r-dr)
            ...
            # print(type(log_rdot))# 这一步已经变成matrix了
    ...
    for j in range(6):
        ...
        # print(np.linspace(0,np.shape(log_rdot)[1]/ros_freq,np.shape(log_rdot)[1]).shape)
        # print( np.reshape(log_rdot[j,:],[-1,]).shape)
        # print(type(log_rdot[j,:]))
        plt.plot(np.linspace(0,np.shape(log_rdot)[1]/ros_freq,np.shape(log_rdot)[1]),   np.reshape(np.array(log_rdot[j,:]),[-1,]) )
        # plt.plot(np.linspace(0,np.shape(log_drdot)[1]/ros_freq,np.shape(log_drdot)[1]),   np.reshape(log_drdot[j,:], [-1,]) )
