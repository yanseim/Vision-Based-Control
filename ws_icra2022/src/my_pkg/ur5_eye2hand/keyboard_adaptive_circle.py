...
def main():
    ...
    initial_state=[145.53,-124.99, -73.39, 23.17, 89.61, 136.68]# 单位是角度deg
    ...
    dx = np.reshape([820,540],[2,1])
    dxdot = np.reshape([0,100*0.2/3*math.pi],[2,1])
    ...
    while not rospy.is_shutdown():
        ...
        if flag_initialized==0:# 先回到初始位置，movej的t参数是不需要的，这里给0 
            ...
            # adaptive
            ut = -z_hat[0]*np.dot( J_pos_inv, np.dot( Js_hat_inv , np.dot(Kp, (x-dx-dxdot) ) ) )
        ...
        t_last = t
        # update dx
        dx =np.array( [[720+100*math.cos(0.2/3*math.pi*(t-t_ready))], [540+100*math.sin(0.2/3*math.pi*(t-t_ready))]]  )
        dxdot = np.array( [[-100*0.2/3*math.pi*math.sin(0.2/3*math.pi*(t-t_ready))], [100*0.2/3*math.pi*math.cos(0.2/3*math.pi*(t-t_ready))]]  )
    ...
    plt.plot(log_dx_array[:,0], log_dx_array[:,1],":",label = 'desired',)
    plt.legend()
